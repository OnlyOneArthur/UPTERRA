...
export default function LaporanSaya() {
  ...
  const filtered = DUMMY_LAPORAN.filter(l => {
    ...
    const matchSearch = l.lokasi.toLowerCase().includes(search.toLowerCase()) ||
      ...
      <div className="mx-auto max-w-md pb-10">
        ...
        <div className="px-5 pt-5">
          ...
          <div className="rounded-[20px] bg-white shadow-[0_4px_14px_rgba(61,168,94,0.12)] border border-[#e8f5ee] mb-4 p-4">
            {/* Total */}
            <div className="mb-3">
              <p className="text-[11px] text-[#aaa] font-medium mb-0.5">Total Laporan</p>
              <p className="text-[26px] font-extrabold text-[#2d2d2d] leading-none">
                {summary.total} <span className="text-[14px] font-bold text-[#2d2d2d]">Laporan</span>
              </p>
              <p className="text-[11px] text-[#aaa] mt-0.5">telah berhasil kamu lapor</p>
            </div>

            {/* Status Pills */}
            <div className="grid grid-cols-4 gap-2">
              {[
                { label: "Terkirim",   count: summary.Dikirim,    emoji: "📤", color: "#1d64d8" },
                { label: "Diproses",   count: summary.Diproses,   emoji: "⚙️", color: "#a16207" },
                { label: "Selesai",    count: summary.Selesai,    emoji: "✅", color: "#15803d" },
                { label: "Dibatalkan", count: summary.Dibatalkan, emoji: "❌", color: "#dc2626" },
              ].map(item => (
                <div key={item.label} className="flex flex-col items-center rounded-[14px] bg-[#f8f8f8] py-2.5 px-1">
                  <span className="text-[16px] mb-1">{item.emoji}</span>
                  <span className="text-[15px] font-extrabold" style={{ color: item.color }}>{item.count}</span>
                  <span className="text-[9px] text-[#aaa] font-medium mt-0.5 text-center leading-tight">{item.label}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="flex flex-col gap-3">
            {filtered.length === 0 ? (
              <div className="text-center py-12 text-[#aaa] text-[13px]">Tidak ada laporan ditemukan.</div>
            ) : (
              filtered.map((laporan, i) => {
                const cfg = STATUS_CONFIG[laporan.status] ?? STATUS_CONFIG["Dikirim"];
                return (
                  <div
                    key={i}
                    onClick={() => navigate("/detail-laporan", { state: laporan })}
                  ...
                  >
                    ...
                    <div className="flex-shrink-0 h-[68px] w-[68px] rounded-[12px] bg-[#e8f5ee] flex items-center justify-center overflow-hidden">
                      {laporan.foto ? (
                        <img src={laporan.foto} alt="laporan" className="h-full w-full object-cover" />
                      ) : (
                    ...
                    <div className="flex flex-col justify-between flex-1 min-w-0">
                      <div>
                        <p className="text-[13px] font-bold text-[#2d2d2d] truncate">{laporan.lokasi}</p>
                        <p className="text-[11px] text-[#aaa] mt-0.5">{laporan.waktu}</p>
                      </div>
                      <div className="flex items-center justify-between mt-2">
                        <p className="text-[10px] text-[#bbb] truncate max-w-[60%]">ID Laporan : {laporan.id}</p>
                        {/* Status Badge */}
                        <span
